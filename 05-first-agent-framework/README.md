# Project 5 — Your first agent with a framework (LangGraph) / Proyecto 5 — Tu primer agente con framework (LangGraph)
...
Online store agent (LangGraph). What do you need?
Type 'quit' to end the session.

You> How much does it cost to buy 2 monitors?

  [Agent node] Asking the model (memory: 1 messages)...
  [Agent node] Model decided: call tool(s): search_product (provider: groq)
  [Tools node] Running search_product with {'term': 'monitor'}
  [Tools node] Result: Products found:
- monitor: $249,999.00 (stock: 8)
  [Agent node] Asking the model (memory: 3 messages)...
  [Agent node] Model decided: call tool(s): calc_total (provider: groq)
  [Tools node] Running calc_total with {'product': 'monitor', 'quantity': 2}
  [Tools node] Result: Product: monitor
Unit price: $249,999.00
Quantity: 2
Subtotal: $499,998.00
VAT (21%): $104,999.58
Total: $604,997.58
  [Agent node] Asking the model (memory: 5 messages)...
  [Agent node] Model decided: reply in text (provider: groq)
Agent> 2 monitors cost $604,997.58 total (21% VAT included).

You> And what was the total of the monitors I just asked about?

  [Agent node] Asking the model (memory: 7 messages)...
  [Agent node] Model decided: reply in text (provider: groq)
Agent> The total for the 2 monitors you asked about is $604,997.58.
```
...
Notá cómo **una sola pregunta dispara dos herramientas en cadena** (buscar y después calcular), y la segunda pregunta se responde **desde la memoria** (sin herramientas).
